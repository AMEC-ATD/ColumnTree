...
folder are mapped to the application's root namespace, 'ColumnTreeDemo'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "ColumnTreeDemo"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'ColumnTreeDemo' in the same way
as `"ColumnTreeDemo/sass/src"`.
